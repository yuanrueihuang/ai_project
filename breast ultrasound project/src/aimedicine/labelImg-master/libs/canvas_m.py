...
class Canvas(QWidget):
    ...
    def mousePressEvent(self, ev):
        if ev.button() == Qt.RightButton:
            self.right_click_pos = self.transformPos(ev.pos())
        else:
            self.right_click_pos = None
    ...
    def findRectanglePointforshape(self):
        ...
        for shape in self.shapes:
            if shape.drawRegion:
                rect_list.append(self.storePoint(shape))
        return rect_list
    ...
    def getRectlangle(self):
        rect_list = []
        for sh in self.shapes:
           if not sh.drawRegion:
                rect = self.covert2Rectangle(sh.points)
    ...
    def set_detect_shape(self, relative_point,  shape_points):
        ...
        for pt in shape_points:
            self.current.addPoint(QPointF(relative_point[0]+pt[0], relative_point[1]+pt[1]))
