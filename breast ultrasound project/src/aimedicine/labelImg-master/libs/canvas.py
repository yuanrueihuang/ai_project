...
from aimedicine import SaveImage
...
class Canvas(QWidget):
    ...
    def capture_predict_area(self):
        ...
        split_img = self.pixmap.copy(rect[0], rect[1], rect[2], rect[3]).toImage()
    ...
    def set_detect_shape(self,bi_rads, relative_point, shape_points):
        ...
        self.detectShape = DetectedShape(label=bi_rads, selectdisplay=self.select_display)
        ...
        for sp in shape_points:
            pt_list =[]
            for pt in sp:
                pt_list.append(QPointF(relative_point[0] + pt[0], relative_point[1] + pt[1]))
    ...
    def set_axis_information(self, relative_point, axis_points, horizontal_len, vertical_len):
        if self.detectShape is not None:
            self.detectShape.set_Horizontal_Line(relative_point,
                                                 axis_points,
                                              horizontal_len)

            self.detectShape.set_Vertical_Line( relative_point,
                                                axis_points,
                                              vertical_len)

    def set_detected_rectangle(self, relative_point, detected_rectangle):
        if self.detectShape is not None:
            self.detectShape.set_Detected_Rectangle(relative_point, detected_rectangle)
    ...
    def findRectanglePointforshape(self):
        ...
        for sh in self.shapes:
            ...
            rect_list.append((rect[0], rect[1], rect[0] + rect[2], rect[1] + rect[3]))
        return rect_list
    ...
    def get_tissue_rect_list(self):
        ...
        for sp in self.shapes:
            ...
            pt_list.append([rect[0], rect[1], rect[0] + rect[2], rect[1] + rect[3]])
                # self.detectShape.addPoint(QPointF(relative_point[0] + pt[0], relative_point[1] + pt[1]))
        return pt_list
