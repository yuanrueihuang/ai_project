...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
       ...
       #                 icon='splitimg', tip=getStr('generateSplitImageDetial'), enabled=False)
        ...
        aipredict = action(getStr('toAIAnalyze'), self.toAIPredict, '',
                       icon='splitimg', tip=getStr('toAIAnalyzeDetial'), enabled=True)
        ...
        self.actions.beginner = (open, opendir, openNextImg, openPrevImg, save, None, create, region, delete, None )
    ...
    def saveAsDICOM(self):
        ...
        try:
            ...
            us_dicom = USDicom.USDicomClass(os.path.join(os.path.abspath('.'),filename[0] +'.dcm'), True)
            img = self.canvas.getImageArray()
            # save image
            us_dicom.setUSImage(img , img.shape[1], img.shape[0])

            # save rectangle
            rect_list= self.canvas.getRectlangle()
        ...
        try:
            ...
            if not test_range_image is None:
                ...
                if (len(r['class_ids']) > 0):
                    ...
                    self.bu_model.region_of_interest(r, check_point, rect_point)

                    self.canvas.set_detect_shape(rect_point[0], self.bu_model.get_side_points() )
    ...
    def loadRegionType(self, shape_list , rect_list, region_type):
        ...
        for rect in rect_list:
            shape = Shape(drawRegion=IsdrawRegion)
    ...
    def loadDCMFile(self, filename):
        ...
        shape_list = []
        rect_box_list = us_dicom.getRegionBox(0x01)  # tissue
        self.loadRegionType(shape_list, rect_box_list, 0x01)

        rect_box_list = us_dicom.getRegionBox(0x02)  # color
        self.loadRegionType(shape_list, rect_box_list,0x02)
        #if self.labelFile:
        # self.loadLabels(self.labelFile.shapes)
